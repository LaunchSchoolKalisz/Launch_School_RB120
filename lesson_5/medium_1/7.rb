...
In the previous exercise, you wrote a number guessing game that determines a secret number between 1 and 100, and 
...
compute a secret number for the game. You should also change the number of guesses allowed so the user can always 
win if she uses a good strategy. You can compute the number of guesses with:
...
Discussion
We've made some minor changes. First, we updated #initialize to handle initialization of the range and the number of 
guesses allowed, which we store in @range and @max_guesses, respectively. We then replaced the RANGE and 
MAX_GUESSES constants in the rest of the code, and deleted the constant declarations.
=end
