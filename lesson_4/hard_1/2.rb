...
Now Alan has asked them to incorporate a new type of vehicle into their system - a Catamaran defined as 
follows:
...
We can move anything shared with the catamaran and wheeled vehicles into a new module (let's call 
Moveable) that we include in both individual classes.

Also, in the initializers of the classes that include the Moveable module, we must reference the setter 
methods that have been defined in Moveable (the fuel capacity and efficiency) using self. notation. 
Otherwise, this would be interpreted as setting local variables that will lose their scope as soon as 
...
and we've moved any code shared between Catamaran and WheeledVehicles to the Moveable module.
=end